  ...
  <div class="game-page">
    ...
    <div class="grid-wrapper" v-if="rows.length">
      <div class="grid" :style="gridStyle" @click="handleGridClick">
        <div v-for="(cell,i) in cells" :key="i" class="cell" :style="cellStyle(cell)" :title="`(${Math.floor(i / gridW)}, ${i % gridW})`"></div>
      </div>
    </div>
    <div class="status" v-if="gameState.gameOver && !gameState.marking">{{ winnerText }}</div>
    <div class="key-hint">Click to place a stone</div>
  </div>
</template>
...
const {
  ...
  startGame, endGame, togglePause, handleGridClick, cellStyle,
